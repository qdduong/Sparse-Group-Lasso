...
from utils import l2, get_splrep, get_regfunc, get_projecting_matrix, return_group_to_index, discard_group, block_wise_descent_fitting, get_coef_exclude_ind, df_dbeta0, df_dtheta0
from scipy.optimize import fmin_l_bfgs_b, newton
import numpy as np
...
class SGL(object):
    def __init__(self, y, X, knots, max_iter=1000, rtol=1e-6):
        self.y = y
        self.X = X
        self.knots = knots
    ...
    def fit(self, u, lbda1, alpha1, lbda2, alpha2):
        ...
        while distance > self.rtol and nb_iter <= self.max_iter:
            ...
            # 1- Should the group be zero-ed out?
                ...
                tmp_coefs_gr[gr] = 0.
                if discard_group(self.y, self.knots, tmp_coefs_gr, self.splrep, self.proj_matrix, u, lbda1, alpha1, gr):
                    self.coef_[gr] = 0.
                # 2- If the group is not zero-ed out, update each component
                else:
                    self.coef_[gr] = block_wise_descent_fitting(self.coef_, self.y, self.knots, self.splrep, self.proj_matrix, u, lbda1, alpha1, gr)
            ...
            # 1- Should the group be zero-ed out?
                tmp_coefs_gr = self.coef_.copy()
                tmp_coefs_gr[gr] = 0.
                if discard_group(self.y, self.knots, tmp_coefs_gr, self.splrep, self.proj_matrix, u, lbda2, alpha2, gr):
                    self.coef_[gr] = 0.
                # 2- If the group is not zero-ed out, update each component
                else:
                    self.coef_[gr] = block_wise_descent_fitting(self.coef_, self.y, self.knots, self.splrep, self.proj_matrix, u, lbda2, alpha2, gr)
            """        
            # estimate beta_0
            ind_beta0 = int(self.groups[0][0])
            beta0_old = self.coef_[ind_beta0]
            coef_excl_beta0 = get_coef_exclude_ind(self.coef_, ind_beta0)
            beta0_new = newton(func=df_dbeta0, x0=beta0_old,
                               args=(coef_excl_beta0, ind_beta0, self.y, self.knots, self.splrep, self.proj_matrix, u))
            ...
            theta0_new = newton(func=df_dtheta0, x0=theta0_old,
                                args=(coef_excl_theta0, ind_theta0, self.y, self.knots, self.splrep, self.proj_matrix, u))
            self.coef_[ind_theta0] = theta0_new
            # update_nb_iter
            nb_iter += 1
            # update_distance
            distance = l2(self.coef_ - coefs_old)/l2(coefs_old)
        return self
    ...
    def fit_predict(self, x, u, lbda1, alpha1, lbda2, alpha2):
        self.define_groups()
        return self.fit(u, lbda1, alpha1, lbda2, alpha2).predict(x)