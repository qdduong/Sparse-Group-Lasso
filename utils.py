...
def block_wise_descent_fitting(coefs, y, knots, splrep, proj_matrix, u, lbda, alpha, indices_group, rtol=1e-6, max_iter=1000):
    ...
    while cond > rtol and l <= max_iter:
        ...
        #-------------------------------------------------------------------------------
        coefs_new = mu_old + l/(l+3)*(mu_new - mu_old)
        tmp_mu = mu_new
        tmp_coefs = coefs_new
        l = l+1
        cond = l2(coefs_new - coefs_old)/l2(coefs_old)
...
def df_dbeta0(x0, coef_k, ind, y, knots, splrep, proj_matrix, u):
    """
        This method returns the partial derivative of Kullback-Leibler divergence with respect to beta_0. 
    """
    x0 = float(x0)
    coefs = get_coef_include_ind(coef_k, x0, ind)
    return partial_f_beta0(coefs, y, knots, splrep, proj_matrix, u)
...
def df_dtheta0(x0, coef_k, ind, y, knots, splrep, proj_matrix, u):
    """
        This method returns the partial derivative of Kullback-Leibler divergence with respect to theta_0.
    """
    x0 = float(x0)
    coefs = get_coef_include_ind(coef_k, x0, ind)
    return partial_f_theta0(coefs, y, knots, splrep, proj_matrix, u)
